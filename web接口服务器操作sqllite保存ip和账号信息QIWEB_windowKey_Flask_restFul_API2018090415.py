...
#跨域访问
def return_ajax_ok(text):
    response = make_response(text)
    response.headers['Access-Control-Allow-Origin'] = '*'
    response.headers['Access-Control-Allow-Methods'] ='PUT,GET,POST,DELETE'# 'POST'
...
def get_emails():
    ...
    # return jsonify({'tasks': tasks})
    #转换id 成url比较直观
    return return_ajax_ok(jsonify({'email': map(make_public_email, emails)}))
    #return jsonify({'email': map(make_public_email, emails)})

@app.route('/qiweb/api/v1.0/emailspages', methods=['GET'])
def get_emailspages():
    emails = []
    conn = sqlite3.connect('windowskey.db')
    qsql='select * from email'
	...
	emails.append(row)
        #emails.append(email)
    conn.close()
    # 我们使用    JSON    数据格式来响应，Flask    的    jsonify    函数从我们的数据结构中
    # return jsonify({'tasks': tasks})
    #转换id 成url比较直观
    return return_ajax_ok(jsonify({'draw':9,'recordsTotal':lengzz ,'recordsFiltered':lengzz,'data':  emails}))
    #return jsonify({'email': map(make_public_email, emails)})
...
@app.route('/add_ip/<ip>', methods=['GET'])
def add_ip(ip):
    nowTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    # 现在
    insert2 = '''
        INSERT INTO ipaddrs(ipaddr,add_date) 
         
         VALUES ( '%s','%s' ) ;'''
    extc_sql(insert2%(ip,nowTime))
    return jsonify({'result': True,'msg':'add ip : %s successfully'%ip})
...
def init_db():
    ...
    createips = '''CREATE TABLE ipaddrs ( 
            id          INTEGER         PRIMARY KEY AUTOINCREMENT
                                        NOT NULL
                                        UNIQUE,
